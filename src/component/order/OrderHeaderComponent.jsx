...
class OrderHeaderComponent extends Component {
    ...
    render() {
        ...
        if (this.state.error !== null || this.state.isLoading) {
            ...
            return (
                <div>
                    <Typography variant="h4" style={{marginLeft: '25%', width: '50%', display: 'inline-block'}}
                                align="center">Nagłówek zamówienia</Typography>
                    ...
                    <TextField variant="outlined" autoFocus select style={{width: '100%'}} label="Typ żywienia"
                               margin="normal"
                    ...
                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                        <DateTimePicker variant="inline" label="Data Zamówienia" name="dataZlecenia" ampm={false}
                                        inputVariant="outlined"
                                        value={this.state.dataZlecenia}
                                        onChange={(date) => this.setState({dataZlecenia: new Date(date)})}
                        ...
                        <DateTimePicker variant="inline" label="Data Zapotrzebowania" name="dataNa" ampm={false}
                                        inputVariant="outlined"
                                        value={this.state.dataNa}
                                        onChange={(date) => this.setState({dataNa: new Date(date)})}
                                        margin="normal" format="dd.MM.yyyy HH:mm" style={{width: '33%'}}
                        />
                    </MuiPickersUtilsProvider>
                    <TextField variant="outlined" autoFocus type="text" disabled style={{width: '33%'}}
                               label="Jednostka Zamawiająca" margin="normal" name="nazwisko"
                               value={this.state.josZamawiajacy.nazwa} onChange={this.onChange}/>

                    <TextField variant="outlined" autoFocus type="text" required style={{width: '33%'}}
                               label="Rozpoznanie" margin="normal" name="rozpoznanie" value={this.state.rozpoznanie}
                    ...
                    <Typography variant="h6" style={{width: '75%', display: 'inline-block'}} align='left'>Aktualny
                        pomiar pacjenta:</Typography>
                    <Button style={{width: '25%'}} variant="contained" color="primary" onClick={() => this.addStudy()}>Dodaj
                        pomiar</Button>

                    {this.state.newActive && (
                        <div style={{width: '100%', marginBottom: '20px'}}>
                            <StudyComponent idPacjenta={this.state.pacjent.id}
                                            updateList={() => this.updateStudy(this.state.pacjent.id)}/>
                        </div>
                    )}
                    {!this.state.newActive && (
                        <div style={pomiarStyle}
                             onClick={() => alert('Domyślnie pobierany najnowższy pomiar dla pacjenta. Można go dodać w danych pacjenta')}>
