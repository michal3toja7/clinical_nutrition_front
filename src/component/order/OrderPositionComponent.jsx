            ...
            return (
                <div style={flexStyle}>
                    <div component={Paper} style={{width: "100%", textB: 'white'}}>
                        <Typography variant="h6" fullwidth="true"
                                    align="left">Preparat {this.props.pozycja}:</Typography>

                        <Autocomplete id={"preparat" + this.state.id} options={this.state.preparaty} name='preparat'
                                      value={this.state.preparat} getOptionLabel={option => (option.nazwa + '')}
                                      onChange={(event, value) => this.setState({preparat: value})}
                        ...
                        <TextField variant="outlined" autoFocus type="number" required style={{width: '50%'}}
                                   label="Objętość (ml)" margin="dense" name="objetosc" value={this.state.objetosc}
                                   onChange={this.onChange}/>

                        <TextField variant="outlined" autoFocus type="number" required style={{width: '50%'}}
                                   label="Co ile Godzin (h)" margin="dense" name="coIleH" value={this.state.coIleH}
                                   onChange={this.onChange}/>

                        <TextField variant="outlined" autoFocus type="number" style={{width: '50%'}}
                                   label="Ilość podań na dobę" margin="dense" name="ilePodan"
                                   value={Math.round(24 / this.state.coIleH, 0)}/>

                        <TextField variant="outlined" autoFocus type="number" style={{width: '50%'}}
                                   label="Ilość ml na dobę" margin="dense" name="iloscMl"
                                   value={this.state.objetosc * Math.round(24 / this.state.coIleH, 0)}/>
                        {this.props.typ === 'DOJ' && (
                            <div>
                                <TextField variant="outlined" autoFocus type="number" style={{width: '50%'}}
                                           label="Czas wlewu (h)" margin="dense" name="czasWlewu"
                                           value={this.state.czasWlewu} onChange={this.onChange}/>

                                <TextField variant="outlined" autoFocus type="number" style={{width: '50%'}}
                                           label="Szybkość przetaczania ml/h" margin="dense" name="uwagi"
                                           value={Math.round(100 * this.state.objetosc / this.state.czasWlewu) / 100}/>

                                <TextField variant="outlined" autoFocus select style={{width: '100%'}}
                                           label="Sposób podania" margin="dense"
                                           name="sposobPodania" value={this.state.sposobPodania}
                                           onChange={this.onChange}>
                                    <MenuItem value={"WLC"}>Wlew ciągły</MenuItem>
                                    <MenuItem value={"BOL"}>Bolus</MenuItem>
                                    <MenuItem value={"WLK"}>Wlew
                                        kroplowy {Math.ceil((this.state.objetosc / this.state.czasWlewu) / 3, 0)} kropli/min</MenuItem>
                                </TextField>
                            </div>
                        )}

                        <TextField variant="outlined" autoFocus type="text" style={{width: '100%'}}
                                   label="Uwagi" margin="dense" name="uwagi" value={this.state.uwagi}
                                   onChange={this.onChange}/>

                        {this.state.saveIsActive && (
                            <Button style={{width: '48%', marginRight: '2%'}} onClick={this.saveOrderPos}
                                    variant="contained" color="primary">Zapisz</Button>)}
                        {this.state.saveIsActive && (
                            <Button style={{width: '48%', marginLeft: '2%'}} onClick={this.deleteOrderPos}
