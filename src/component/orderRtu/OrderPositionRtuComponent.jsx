                ...
                .then(result => {
                    if (result.error !== undefined) {
                        ...
                        if (result.data[0] !== undefined) {
                            this.setState({
                                dodatki: result.data.sort((a, b) => a.dodatek.nazwa.localeCompare(b.dodatek.nazwa)),
                                newActive: false
                            })
            ...
            return (
                <div style={flexStyle}>
                    <div component={Paper} style={{width: "100%", textB: 'white'}}>
                        ...
                        <Autocomplete id="worekPreparat" options={this.state.worki} name='worekPreparat'
                                      value={this.state.worekPreparat}
                        ...
                        <TextField variant="outlined" autoFocus type="number" style={{width: '100%'}}
                                   label="Czas wlewu (h)" margin="dense" name="czasWlewu" value={this.state.czasWlewu}
                        ...
                        {this.state.dodatki.map((row, index) => (
                            <div key={row.id}>
                                <OrderPositionSuppComponent key={row.id} updateList={() => this.reloadOrderSupp()}
                                                            orderPosSupp={row}/>
                            </div>
                        ...
                        <div>
                            {this.state.newActive && (
                                <div>
                                    <OrderPositionSuppComponent zamowieniePozRtuId={this.state.id}
                                                                updateList={() => this.reloadOrderSupp()}/>
                                </div>
                            )}
                        </div>


                        <Button style={{width: '50%', marginInline: '50%'}} disabled={this.state.id === ''}
                                onClick={() => this.addSupplement()} variant="contained" color="primary">Dodaj
                            dodatek</Button>
                        <hr/>
                        ...
                        {this.state.saveIsActive && (
                            <Button style={{width: '48%', marginRight: '2%'}} onClick={this.saveOrderPosRtu}
                                    variant="contained" color="primary">Zapisz</Button>)}
                        {this.state.saveIsActive && (
                            <Button style={{width: '48%', marginLeft: '2%'}} onClick={this.deleteOrderPosRtu}
